...
репозитория и не видят рабочих деревьев друг друга. Общее у них — история git
и этот файл.

Почему JSONL, а не база или сервис: доска обязана читаться человеком без
инструментов. Она лежит в git, видна в `git log`, открывается блокнотом.
Ровно по той же причине так устроен `.ai-loop/ledger.jsonl`.

Хеш-цепочки здесь намеренно нет. Ledger доказывает, что решение было принято, и
подделка там меняет смысл записи. Доска — это переписка: подделывать её незачем,
а цена лишней церемонии — что ею перестанут пользоваться.
...
BOARD = Path(__file__).resolve().parent / "board.jsonl"
