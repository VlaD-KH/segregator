...
async def test_досье_объясняет_каждую_цифру(собранная_база) -> None:
    ...
    # Объяснение берётся из профиля, а не из общего словаря: один и тот же сигнал
    # означает разное у ресторана и у салона красоты.
    assert "комиссия ~30% с каждого заказа" in текст
